...
The data analysis workflow typically follows these key steps:
...
Having already collected data from WRDS and cleaned it to retain only relevant companies—excluding outliers and missing values—we now move into **Step 4: Exploratory Data Analysis (EDA)**. This stage focuses on visualizing the processed financial data to identify patterns, trends, and relationships that inform subsequent analysis.

### 

This section utilizes several key R packages:

-   **`ggplot2`** and **`corrplot`** for data visualization.( check for [ggplot](https://ggplot2.tidyverse.org/) and [corrplot](https://www.sthda.com/english/wiki/visualize-correlation-matrix-using-correlogram?utm_source=chatgpt.com) library tutorial in the provided links)

-   **`modelsummary`**, **`kableExtra`**, and **`formattable`** for formatting tables and outputs. ( To learn more about [modelsummary](https://modelsummary.com/.) , [kableExtra](https://bookdown.org/yihui/rmarkdown-cookbook/kableextra.html), and [formattable](https://www.r-bloggers.com/2018/11/make-beautiful-tables-with-the-formattable-package/) click on provided links)

All visual outputs are exported in two formats: PDF for publication-quality outputs (e.g., academic papers), and PNG for use in presentations or reports.
...
### 1. Losses by Industry (FF12 Classification)

Firms are grouped by Fama-French 12 (FF12) industry categories. For each category, we calculate the proportion of firms reporting financial losses. These figures are visualized using a horizontal bar chart. The chart highlights which sectors tend to have higher frequencies of loss-making firms, offering insight into industry-specific financial vulnerabilities.
...
### 2. Losses by Firm Size Over Time

Firms are categorized into **size quintiles** based on market value within each calendar year. The proportion of loss-making firms is then computed annually for each quintile. These trends are displayed using line charts, enabling comparison of financial performance across different firm sizes over time.
...
A subset of key financial metrics is selected, including: Return on Assets (current and future), Loss indicators, R&D intensity, Total Assets, Market Value of Equity. A correlation matrix is generated and visualized via a heatmap to reveal relationships among these variables, helping identify potential multicollinearity or associations worth further exploration.
...
### 4. Annual Regressions – Earnings Persistence

Yearly regressions are run to evaluate the relationship between current and future earnings. These regressions are performed separately for firms with and without reported losses. Confidence bands are included to show estimation uncertainty. This analysis investigates earnings persistence, which is crucial for financial forecasting and valuation.

Files are stored in a designated `output/` directory using standardized naming conventions for easy reference and reproducibility.
