...
**Hypothesis 1 (H1) \\label{hypo} :** *Ceteris paribus, earnings are less persistent for loss firms than profit firms.*
...
Here is the regression equation that we use to test [@hypo], which was defined in [Section @sec-background]:\
Footnote: Note that since we have dynamically defined and referred to \[H\@hyp-losses\], you can click on it to jump to the place in the text where H1 is defined. We can do the same thing for \[RQ\@researchq-losses\].
...
-   @tbl-regression provides an example regression table, created in R.

    Here are some examples of inline table references, including references to the relevant equation and hypothesis. @tbl-corr and @tbl-regression present the results from estimating Eq. (\ref{eq:losses}). The significantly negative coefficients on \$ LOSS \times ROA\_{t}\$ in Columns (4) and (5) provide some evidence consistent with H\ref{hyp:losses}. However, it seems to be important to control for firm characteristics when testing this hypothesis.
...
combined_corr |>
  ...
  kable_styling(
    latex_options = c("striped", "hold_position","scale_down"), # LaTeX-specific styling
    full_width = FALSE
  )
```
...
# Custom function to format the number of observations
nobs_fmt <- function(x) {
  out <- formattable::comma(x, digits = 0)
  return(out)
}

# Custom function to check if controls are in the model
