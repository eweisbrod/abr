...
Tables can also be generated using R chunks, as shown in @tbl-simple example. One benefit of using Quarto is that you could add inline code to your document referencing, for example, coefficients. The coefficient on $ROA_t$ in @tbl-simple is `r broom::tidy(models$Base) |> 
  filter(term == "roa") |> 
  select(estimate) |> 
  as.numeric() |> 
  format(digits = 3)`\.
